...
def main():
    bike_raw=pd.read_csv('../data/p1ch4/bike-sharing-dataset/hour-fixed.csv',dtype=np.float32,converters={1:lambda x:float(x[8:10])})
    ...
    daily_weather_onehot=torch.zeros(daily_bike.shape[0],4,daily_bike.shape[2])
    daily_weather_onehot.scatter_(1,daily_bike[:,9,:].long().unsqueeze(1)-1,1.0)
